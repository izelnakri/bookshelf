...
dataInMemory.push({
  ...
  permalink: '/books/javascript-the-good-parts',
...
dataInMemory.push({
  ...
  permalink: '/books/the-little-prince',
...
dataInMemory.push({
  ...
  permalink: '/books/europe-on-a-shoestring',
...
dataInMemory.push({
  ...
  permalink: '/books/security-analysis',
...
function findBook(slug) {
  ...
  for (var i = 0; i < dataInMemory.length; i++) {
    if (dataInMemory[i].permalink === '/books/' + slug) {
...
app.get('/books', function(req, res) {
  ...
  res.send(pug.renderFile(__dirname + '/views/index.pug', { books: dataInMemory }));
});

app.get('/books/*', function(req, res) {
  var foundBook = findBook(req.params[0]);
  res.send(pug.renderFile(__dirname + '/views/book.pug', { book: foundBook }));
});
